...
//  Заголовки	 - Структура, Строка	 - Заголовки сообщений в формате "Заголовок1: значение; Заголовок2: Значение"
...
Процедура ОтправитьСообщения(МассивСообщений) Экспорт
	...
	Для каждого Сообщение Из МассивСообщений Цикл
		Данные = Закодировать(Сообщение.Данные);
		Ключ = Закодировать(Сообщение.Ключ);
		
		Если ТипЗнч(Сообщение.Заголовки) = Тип("Строка") Тогда
			Заголовки = Закодировать(Сообщение.Заголовки);
		Иначе
			СтрокаЗаголовков = ЗаголовкиВСтроку(Сообщение.Заголовки);
			Заголовки = Закодировать(СтрокаЗаголовков);
		КонецЕсли;
		
		Компонента.Отправить(Настройка.Топик, Данные, Ключ, Заголовки, Сообщение.Партиция, Сообщение.Идентификатор);
...
// Читает пакет сообщений из Кафки с метаданными. Если метаданные не требуются, лучше воспользоваться 
//  методами ПрочитатьСледующееСообщение() и ПрочитатьСообщение()
//
// Параметры:
...
Функция ПрочитатьСообщения(КоличествоСообщенийКЧтению) Экспорт
	...
	Пока МассивСообщений.Количество() < КоличествоСообщенийКЧтению 
		И ПрочитатьСледующееСообщение() Цикл
		
		ТекстСообщения = ПрочитатьСообщение();
		Ключ = ПрочитатьКлючСообщения();
		Заголовки = ПрочитатьЗаголовкиСообщения();
		МетаданныеJSON = ПрочитатьМетаданныеСообщения();
		Мета = ДанныеИзJSON(МетаданныеJSON);
		
		НовоеСообщение = НовоеПрочитанноеСообщение(ТекстСообщения, Ключ, Заголовки, Мета.partition, Мета.offset);
...
// 
// Возвращаемое значение:
//  Булево - Истина, если сообщение прочитано
//
Функция ПрочитатьСледующееСообщение() Экспорт
...
// Возвращает текст сообщения из Кафки
// 
// Возвращаемое значение:
...
Функция ПрочитатьСообщение() Экспорт
	...
	Если Компонента.Отказ Тогда
		ТекстСообщения = СтрШаблон("Не удалось прочитать сообщение: %1", Компонента.ОписаниеОшибки);
		ВызватьИсключение ТекстСообщения;
	КонецЕсли;
	
	ТекстСообщения = Раскодировать(ДанныеСообщения);
	
	Возврат ТекстСообщения;
	
КонецФункции

// Возвращает ключ текущего прочитанного сообщения из Кафки
// 
// Возвращаемое значение:
//  Строка - Ключ сообщения
//
Функция ПрочитатьКлючСообщения() Экспорт

	ДанныеКлюча = Компонента.КлючСообщения();
	Если Компонента.Отказ Тогда
		ТекстСообщения = СтрШаблон("Не удалось прочитать ключ сообщения: %1", Компонента.ОписаниеОшибки);
		ВызватьИсключение ТекстСообщения;
	КонецЕсли;
	
	Ключ = Раскодировать(ДанныеКлюча);
	
	Возврат Ключ;
	
КонецФункции

// Возвращает заголовки текущего прочитанного сообщения из Кафки
// 
// Возвращаемое значение:
//  Строка - Строка заголовков в формате "Заголовок1: значение; Заголовок2: Значение"
//
Функция ПрочитатьЗаголовкиСообщения() Экспорт

	ДанныеЗаголовков = Компонента.ЗаголовкиСообщения();
	Если Компонента.Отказ Тогда
		ТекстСообщения = СтрШаблон("Не удалось прочитать заголовки сообщения: %1", Компонента.ОписаниеОшибки);
		ВызватьИсключение ТекстСообщения;
	КонецЕсли;
	
	СтрокаЗаголовков = Раскодировать(ДанныеЗаголовков);
	Заголовки = ЗаголовкиИзСтроки(СтрокаЗаголовков);
...
Процедура ИзменитьОффсет(Партиция, Оффсет) Экспорт
	...
	Компонента.ИзменитьОффсет(Настройка.Топик, Партиция, Оффсет);
	Если Компонента.Отказ Тогда
...
Функция ПолучитьНастройку(ИдентификаторНастройки)
	...
	Настройка = Новый Структура("Идентификатор, Отправка, Получение, Брокер, Топик, ГруппаКонсюмера,
		|ТаймаутОпераций, КодировкаТекста, АвтоКоммит, УровеньЛогирования, КоличествоХранимыхЛогФайлов, 
	...
	Запрос.Текст = 
		...
		|	НастройкиКафки.ТаймаутОпераций КАК ТаймаутОпераций,
		|	НастройкиКафки.КодировкаТекста КАК КодировкаТекста,
...
Процедура ПодключитьКомпоненту()
	...
	// +++ DEBUG
	//АдресКомпоненты = "/home/sand/work/rdkafka-1c/build/Desktop-Debug/libRdKafka1C.so";
...
Процедура ИнициализироватьПродюсера()
	...
	Компонента.ТаймаутОпераций = Настройка.ТаймаутОпераций;
...
Функция НовоеПрочитанноеСообщение(Данные, Ключ, Заголовки, Партиция, Оффсет) Экспорт
	
	Сообщение = Новый Структура("Данные, Ключ, Заголовки, Партиция, Оффсет");
	Сообщение.Данные = Данные;
	Сообщение.Ключ = Ключ;
	Сообщение.Заголовки = Заголовки;
	Сообщение.Партиция = Партиция;
	Сообщение.Оффсет = Оффсет;
...
Процедура УстановитьПараметр(Имя, Значение)
	...
	Если Компонента.Отказ Тогда
		// Значение параметра в текст не подставляется: среди параметров есть секреты (sasl.password и т.п.)
		ТекстСообщения = СтрШаблон("Не удалось установить параметр компоненты %1: %2", 
			Имя, Компонента.ОписаниеОшибки);
		ВызватьИсключение ТекстСообщения;
	КонецЕсли;
...
#Область РаботаКодировками

Функция Раскодировать(ДвоичныеДанные)
	
	ЧтениеДанных = Новый ЧтениеДанных(ДвоичныеДанные, Настройка.КодировкаТекста);
...
Функция Закодировать(Строка)
	
	ПотокВПамяти = Новый ПотокВПамяти();
	
	ЗаписьДанных = Новый ЗаписьДанных(ПотокВПамяти, Настройка.КодировкаТекста);
...
Функция ДанныеИзJSON(СтрокаJSON, ИменаСвойствСоЗначениемДата = Неопределено)
	...
	Попытка
		Данные = ПрочитатьJSON(ЧтениеJSON, Ложь, ИменаСвойствСоЗначениемДата);
	Исключение	    
		ТекстСообщения = СтрШаблон("Не удалось десериализовать данные из JSON: %1", КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
		ВызватьИсключение ТекстСообщения;
	КонецПопытки;
...
Функция ДанныеВJSON(Данные)
	...
	Исключение	    
		ТекстСообщения = СтрШаблон("Не удалось сериализовать данные в JSON: %1", КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
		ВызватьИсключение ТекстСообщения;
	КонецПопытки;
...
Функция ЗаголовкиВСтроку(Заголовки)
	...
	Для каждого КлючЗначение Из Заголовки Цикл
		СтрокаЗаголовков = СтрокаЗаголовков + СтрШаблон("%1:%2;", КлючЗначение.Ключ, КлючЗначение.Значение);
	КонецЦикла;
...
Функция ЗаголовкиИзСтроки(Знач СтрокаЗаголовков)
	
	Заголовки = Новый Структура();
	
	МассивСтрок = СтрРазделить(СтрокаЗаголовков, ";", Ложь);
	Для каждого СтрокаКлючЗначение Из МассивСтрок Цикл
		КлючЗначение = СтрРазделить(СтрокаКлючЗначение, ":");
		Если КлючЗначение.Количество() = 2 Тогда
			Заголовки.Вставить(КлючЗначение.Получить(0), КлючЗначение.Получить(1));
		Иначе
			ВызватьИсключение "Не верный формат строки заголовков сообщения";
		КонецЕсли;
	КонецЦикла;
